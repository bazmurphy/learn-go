...
func sendSMSToCouple(msgToCustomer, msgToSpouse string) (float64, error) {
	// call the sendSMS function and pass in the msgToCustomer argument
	// receive the value and the error from the function
	costForCustomer, err := sendSMS(msgToCustomer)
	if err != nil {
		return 0.0, err
	}
	// call the sendSMS function and pass in the msgToSpouse argument
	// receive the value and the error from the function
	costForSpouse, err := sendSMS(msgToSpouse)
	if err != nil {
		return 0.0, err
	}
	// return the combined cost, and nil as the error
