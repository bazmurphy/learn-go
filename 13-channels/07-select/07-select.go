...
// select statements are unique to channels
// they let us listen on two different channels and execute a block of code for the channel that sends a value the soonest
// so one goroutine can process events from multiple channels at the same time using select

func logMessages(chEmails, chSms chan string) {
	// infinite for loop
	for {
		// select syntax
		// we create multiple cases, one for each channel that we are listening/reading values from
		select {
		// case 1
		case email, ok := <-chEmails:
			if !ok {
				// if there are no more values in the channel break out of the loop
				return
			}
			logEmail(email)
		// case 2
