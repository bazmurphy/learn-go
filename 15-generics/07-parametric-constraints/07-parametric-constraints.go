...
// We can use type parameters in interface definitions as well

// customer is the shared interface for "user" and "org" so we can use that as our generic type
type biller[C customer] interface {
	// charge takes in a customer, and returns a bill
	Charge(C) bill
	// name takes no parameters, and returns a string
	Name() string
}
