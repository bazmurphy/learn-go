...
// 		firstLetterRune := rune(name[0])
// 		if _, ok := nameCounts[firstLetterRune]; !ok {
// 			nameCounts[firstLetterRune] = make(map[string]int)
// 		}
// 		nameCounts[firstLetterRune][name] += 1
// 	}
// 	return nameCounts
// }
...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		...
		// when you index into a string in Go then it is a Byte type
		// so we have to cast it to a Rune type
