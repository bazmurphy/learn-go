...
func logAndDelete(users map[string]user, name string) (log string) {
	// defer allows us to execute code at the end of the current function
	// commonly used as a cleanup step

	// in this case we can put the defer at the top of the function
	// we call this delete function right before logAndDelete returns
	defer delete(users, name)

	user, ok := users[name]

	// (!) we are calling delete() three different times:
	if !ok {
		// delete(users, name)
		return logNotFound
	}
